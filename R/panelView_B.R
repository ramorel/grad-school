panelView_B <- function (data, formula = NULL, Y = NULL, D = NULL, X = NULL, 
          index, na.rm = TRUE, ignore.treat = FALSE, outcome.type = "continuous", 
          type = "treat", by.group = FALSE, by.timing = FALSE, theme.bw = FALSE, 
          xlim = NULL, ylim = NULL, xlab = NULL, ylab = NULL, gridOff = FALSE, 
          legendOff = FALSE, legend.labs = NULL, main = NULL, pre.post = FALSE, 
          id = NULL, show.id = NULL, color = NULL, axis.adjust = FALSE, 
          axis.lab = "both", axis.lab.gap = c(0, 0), shade.post = TRUE, 
          cex.main = 15, cex.main.sub = 12, cex.axis = 8, cex.lab = 12, 
          cex.legend = 12, background = NULL, line.width = 0.5) 
{
  ...
  for (i in 1:length(varnames)) {
    if (!varnames[i] %in% colnames(data)) {
      stop(paste("Variable \"", varnames[i], "\" is not in the dataset.", 
                 sep = ""))
    }
  }
  ...
  if (ignore.treat == 0) {
    ...
    d.bi <- d.level[1] == 0 & d.level[2] == 1 & length(d.level) == 
      2
    if (length(d.level) == 1) {
  ...
  if (dim(data)[1] != TT * N) {
    ...
    for (i in 1:dim(data)[1]) {
      if (!is.null(Yname)) {
        Y[data[i, index.time], data[i, index.id]] <- data[i, 
                                                          Yname]
      }
      if (ignore.treat == 0) {
        D[data[i, index.time], data[i, index.id]] <- data[i, 
                                                          Dname]
      }
  ...
  if (ignore.treat == 0 && d.bi == 1) {
    ...
    D <- ifelse(D > 0, 1, 0)
    tr.pos <- which(D[TT, ] == 1)
    T0 <- apply(D == 0, 2, sum)[tr.pos]
    co.total <- co.total.all[tr.pos]
    DID <- length(unique(T0)) == 1
    if (sum(abs(D.old[which(I == 1)] - D[which(I == 1)])) == 
        0) {
      by.group <- by.group
      FEmode <- 0
    }
  ...
  if (ignore.treat == 0 && d.bi == 1) {
    ...
    if (length(d.level) > 0 && type == "treat") {
      obs.missing <- D
  ...
  if (is.logical(legendOff) == FALSE & is.numeric(legendOff) == 
      FALSE) {
    stop("\"legendOff\" is not a logical flag.")
  }
  if (is.logical(gridOff) == FALSE & is.numeric(gridOff) == 
      FALSE) {
  ...
  if (x.gap != 0) {
    T.b <- seq(from = 1, to = length(show), by = (x.gap + 
                                                    1))
  }
  if (legendOff == TRUE) {
    legend.pos <- "none"
  ...
  if (type == "outcome") {
    ...
    if (is.null(color) == TRUE) {
      ...
      if (ignore.treat == 0) {
        if (FEmode == 1) {
          ...
          else if (by.group == TRUE & length(color) != 
                   2) {
            stop("Length of \"color\" should be equal to 2.\n")
          }
          else if (by.group == FALSE & length(color) == 
                   3) {
    ...
    if (ignore.treat == 1) {
      data <- cbind.data.frame(time = rep(time[show], 
                                          N), outcome = c(Y[show, ]), type = rep("co", 
                                                                                 (N * nT)), id = rep(1:N, each = nT))
      ...
      p <- p + theme(legend.position = legend.pos, axis.text.x = element_text(angle = angle, 
                                                                              hjust = x.h, vjust = x.h), plot.title = element_text(size = cex.main, 
                                                                                                                                   hjust = 0.5, face = "bold", margin = margin(8, 
                                                                                                                                                                               0, 8, 0)))
      if (outcome.type == "continuous") {
        p <- p + geom_line(aes(time, outcome, colour = type, 
                               size = type, linetype = type, group = id))
        set.limits = "co"
        ...
        p <- p + scale_colour_manual(limits = set.limits, 
                                     labels = set.labels, values = set.colors) + 
          scale_linetype_manual(limits = set.limits, 
                                labels = set.labels, values = set.linetypes) + 
          scale_size_manual(limits = set.limits, labels = set.labels, 
                            values = set.linewidth) + guides(linetype = guide_legend(title = NULL, 
                                                                                     ncol = labels.ncol), colour = guide_legend(title = NULL, 
                                                                                                                                ncol = labels.ncol), size = guide_legend(title = NULL, 
                                                                                                                                                                         ncol = labels.ncol))
      }
      else {
        ...
        p <- p + scale_colour_manual(limits = set.limits, 
                                     labels = set.labels, values = set.colors) + 
          scale_shape_manual(limits = set.limits, labels = set.labels, 
                             values = set.shapes) + guides(colour = guide_legend(title = NULL, 
                                                                                 ncol = labels.ncol), shape = guide_legend(title = NULL, 
                                                                                                                           ncol = labels.ncol))
      }
      if (!is.numeric(time.label)) {
        p <- p + scale_x_continuous(expand = c(0, 0), 
                                    breaks = show[T.b], labels = time.label[T.b])
      }
    ...
    else if (ignore.treat == 0 && by.group == FALSE) {
      if (outcome.type == "continuous") {
        if (FEmode == 0) {
          ...
          time.pst <- time.pst[which(c(pst[show, ]) == 
                                       1)]
          Y.tr.pst <- c(Y.tr[show, ])[which(pst[show, 
                                                ] == 1)]
          id.tr.pst <- matrix(rep(1:Ntr, each = TT), 
                              TT, Ntr, byrow = FALSE)[show, ]
          id.tr.pst <- c(id.tr.pst)[which(pst[show, 
                                              ] == 1)]
          data <- cbind.data.frame(time = c(rep(time[show], 
                                                N), time.pst), outcome = c(c(Y.tr[show, 
                                                                                  ]), c(Y.co[show, ]), Y.tr.pst), type = c(rep("tr", 
                                                                                                                               (Ntr * nT)), rep("co", (Nco * nT)), rep("tr.pst", 
                                                                                                                                                                       length(Y.tr.pst))), id = c(rep(1:N, each = nT), 
                                                                                                                                                                                                  id.tr.pst * (-1)))
          set.limits = c("co", "tr", "tr.pst")
          set.colors = c(raw.color[1], raw.color[2], 
                         raw.color[3])
          set.linetypes = c("solid", "solid", "solid")
          set.linewidth = rep(line.width, length(set.colors))
          if (!is.null(legend.labs)) {
            if (length(legend.labs) != 3) {
              warning("Incorrect number of labels in the legend. Using default.\n")
              set.labels <- c("Controls", "Treated (Pre)", 
                              "Treated (Post)")
            }
            else {
              set.labels <- legend.labs
          ...
          for (i in 1:N) {
            if (sum(is.na(Y.trt.show[, i])) != nT) {
              ut.id <- c(ut.id, rep(i, nT - sum(is.na(Y.trt.show[, 
                                                                 i]))))
              ut.time <- c(ut.time, time.trt.show[which(!is.na(Y.trt.show[, 
                                                                          i]))])
            }
          }
          data <- cbind.data.frame(time = c(rep(time[show], 
                                                N), ut.time), outcome = c(c(Y[show, ]), 
                                                                          c(Y.trt.show[which(!is.na(Y.trt.show))])), 
                                   type = c(rep("co", (N * nT)), rep("tr", 
                                                                     length(ut.id))), id = c(rep(1:N, each = nT), 
                                                                                             ut.id))
          set.limits = c("co", "tr")
        ...
        if (DID == TRUE && Ntr >= 1) {
          if (time.bf >= min(show) && time.bf <= max(show)) {
            p <- p + geom_vline(xintercept = time.bf, 
                                colour = "white", size = 2)
            if (shade.post == TRUE) {
              p <- p + annotate("rect", xmin = time.bf, 
        ...
        data <- cbind.data.frame(time = rep(time[show], 
                                            N), outcome = factor(c(Y[show, ])), type = c(obs.missing[show, 
                                                                                                     ]))
        data <- na.omit(data)
        data$type <- factor(data$type, levels = c(-1, 
                                                  0, 1), labels = c("co", "tr", "tr.pst"))
        p <- ggplot(data) + xlab(xlab) + ylab(ylab)
        if (theme.bw == TRUE) {
          p <- p + theme_bw()
        }
      ...
      if (1 %in% unit.type) {
        ...
        Nco <- length(co.pos)
        data1 <- cbind.data.frame(time = c(rep(time[show], 
                                               Nco)), outcome = c(Y[show, co.pos]), type = c(rep("co", 
                                                                                                 (Nco * nT))), id = c(rep(1:Nco, each = nT)))
        limits1 <- c("co", "tr")
      ...
      if (2 %in% unit.type) {
        ...
        Ntr <- length(tr.pos)
        data2 <- cbind.data.frame(time = c(rep(time[show], 
                                               Ntr)), outcome = c(Y[show, tr.pos]), type = c(rep("tr", 
                                                                                                 (Ntr * nT))), id = c(rep(1:Ntr, each = nT)))
        limits2 <- c("co", "tr")
      ...
      if (3 %in% unit.type) {
        ...
        if (outcome.type == "continuous") {
          ...
          data3 <- cbind.data.frame(time = c(rep(time[show], 
                                                 Nrv), ut.time), outcome = c(c(Y[show, rv.pos]), 
                                                                             c(Y.trt.show[which(!is.na(Y.trt.show))])), 
                                    type = c(rep("co", (Nrv * nT)), rep("tr", 
                                                                        length(ut.id))), id = c(rep(1:Nrv, each = nT), 
                                                                                                ut.id))
        }
        else {
          data3 <- cbind.data.frame(time = c(rep(time[show], 
                                                 Nrv)), outcome = c(Y[show, rv.pos]), type = c(obs.missing[show, 
                                                                                                           ]), id = c(rep(1:Nrv, each = nT)))
          data3$type <- factor(data3$type, levels = c(-1, 
                                                      1), labels = c("co", "tr"))
        }
        limits3 <- c("co", "tr")
      ...
      subplot <- function(data, limits, labels, colors, 
                          main, outcome.type, theme.bw) {
        if (outcome.type == "discrete") {
          data$outcome <- factor(data$outcome)
          data <- na.omit(data)
        }
        ...
        set.linewidth = rep(line.width, length(limits))
        if (theme.bw == TRUE) {
          p <- p + theme_bw() + theme(panel.grid.major = element_blank(), 
                                      panel.grid.minor = element_blank(), axis.text.x = element_text(angle = angle, 
                                                                                                     hjust = x.h, vjust = x.h), plot.title = element_text(size = cex.main, 
                                                                                                                                                          hjust = 0.5, face = "bold", margin = margin(8, 
                                                                                                                                                                                                      0, 8, 0)))
        }
        ...
        if (outcome.type == "continuous") {
          ...
          p <- p + scale_colour_manual(limits = set.limits, 
            ...
            scale_size_manual(limits = set.limits, labels = set.labels, 
                              values = set.linewidth) + guides(linetype = guide_legend(title = NULL, 
                                                                                       nrow = 1), colour = guide_legend(title = NULL, 
                                                                                                                        nrow = 1), size = guide_legend(title = NULL, 
                                                                                                                                                       nrow = 1))
        }
        else {
          p <- p + geom_jitter(width = 0.15, height = 0.15, 
                               shape = 1, aes(x = time, y = outcome, colour = type))
          p <- p + scale_colour_manual(limits = set.limits, 
                                       labels = set.labels, values = set.colors) + 
        ...
        if (!is.numeric(time.label)) {
          p <- p + scale_x_continuous(expand = c(0, 
                                                 0), breaks = show[T.b], labels = time.label[T.b])
        }
        if (is.null(main) == TRUE) {
          p <- p + ggtitle("Raw Data")
        }
      ...
      if (length(unique(unit.type)) == 1) {
        if (1 %in% unit.type) {
          p1 <- subplot(data1, limits1, labels1, colors1, 
                        main1, outcome.type, theme.bw)
          if (legend.pos != "none") {
            suppressWarnings(g <- ggplotGrob(p1 + theme(legend.position = "bottom"))$grobs)
            legend <- g[[which(sapply(g, function(x) x$name) == 
                                 "guide-box")]]
            suppressWarnings(grid.arrange(arrangeGrob(p1 + 
                                                        theme(legend.position = "none"), legend, 
                                                      nrow = 2, heights = c(1, 1/5)), top = textGrob(main, 
                                                                                                     gp = gpar(fontsize = cex.main.top, font = 2))))
          }
          else {
            suppressWarnings(grid.arrange(p1 + theme(legend.position = "none"), 
                                          top = textGrob(main, gp = gpar(fontsize = cex.main.top, 
                                                                         font = 2))))
          }
        }
        else if (2 %in% unit.type) {
          p2 <- subplot(data2, limits2, labels2, colors2, 
                        main2, outcome.type, theme.bw)
          if (legend.pos != "none") {
            suppressWarnings(g <- ggplotGrob(p2 + theme(legend.position = "bottom"))$grobs)
            legend <- g[[which(sapply(g, function(x) x$name) == 
                                 "guide-box")]]
            suppressWarnings(grid.arrange(arrangeGrob(p2 + 
                                                        theme(legend.position = "none"), legend, 
                                                      nrow = 2, heights = c(1, 1/5)), top = textGrob(main, 
                                                                                                     gp = gpar(fontsize = cex.main.top, font = 2))))
          }
          ...
          p3 <- subplot(data3, limits3, labels3, colors3, 
                        main3, outcome.type, theme.bw)
          if (legend.pos != "none") {
            suppressWarnings(g <- ggplotGrob(p3 + theme(legend.position = "bottom"))$grobs)
            legend <- g[[which(sapply(g, function(x) x$name) == 
                                 "guide-box")]]
            suppressWarnings(grid.arrange(arrangeGrob(p3 + 
                                                        theme(legend.position = "none"), legend, 
                                                      nrow = 2, heights = c(1, 1/5)), top = textGrob(main, 
                                                                                                     gp = gpar(fontsize = cex.main.top, font = 2))))
          }
        ...
        if (!1 %in% unit.type) {
          ...
          if (legend.pos != "none") {
            ...
            suppressWarnings(grid.arrange(arrangeGrob(p2 + 
                                                        ...
                                                        theme(legend.position = "none"), legend, 
                                                      nrow = 3, heights = c(1, 1, 1/5)), top = textGrob(main, 
                                                                                                        gp = gpar(fontsize = cex.main.top, font = 2))))
          }
          else {
            suppressWarnings(grid.arrange(p2 + theme(legend.position = "none"), 
        ...
        if (legend.pos != "none") {
          ...
          legend <- g[[which(sapply(g, function(x) x$name) == 
                               "guide-box")]]
          suppressWarnings(grid.arrange(arrangeGrob(p1 + 
                                                      theme(legend.position = "none"), p2 + theme(legend.position = "none"), 
                                                    p3 + theme(legend.position = "none"), legend, 
                                                    nrow = 4, heights = c(1, 1, 1, 1/5)), top = textGrob(main, 
                                                                                                         gp = gpar(fontsize = cex.main.top, font = 2))))
        }
        else {
          suppressWarnings(grid.arrange(p1 + theme(legend.position = "none"), 
                                        p2 + theme(legend.position = "none"), p3 + 
                                          theme(legend.position = "none"), top = textGrob(main, 
                                                                                          gp = gpar(fontsize = cex.main.top, font = 2))))
        }
      }
    }
    ...
    if (d.bi == FALSE && ignore.treat == 0) {
      tr.col <- c("#FAFAD2", "#ADFF2F", "#87CEFA", "#1874CD", 
                  "#00008B")
      if (length(d.level) <= 5) {
        for (i in 1:length(d.level)) {
          breaks <- c(breaks, d.level[i])
          label <- c(label, paste("Treatment level: ", 
                                  d.level[i], sep = ""))
        }
        if (length(d.level) == 2) {
          col <- tr.col[c(1, 5)]
      ...
      if (length(id) > 1 && ignore.treat == 0 && d.bi == 
          TRUE) {
        if (by.timing == TRUE) {
          ...
          dataT0 <- dataT0[order(dataT0[, "T0"], dataT0[, 
                                                        "co.total"]), ]
          tr.seq <- dataT0[, "id"]
    ...
    if (!is.null(color)) {
      if (length(d.level) <= 5) {
        if (length(col) == length(color)) {
          ...
          stop(paste("Length of \"color\" shold be equal to ", 
                     length(col), ".\n", sep = ""))
        }
      }
      else {
    ...
    if (!is.null(legend.labs)) {
      if (length(d.level) <= 5) {
        if (length(legend.labs) != length(label)) {
          ...
          cat(paste("Specified labels in the order of: ", 
                    paste(label, collapse = ", "), ".\n", sep = ""))
          label <- legend.labs
        }
      }
    ...
    res <- c(m)
    data <- cbind.data.frame(units = units, period = period, 
                             res = res)
    data <- na.omit(data)
    if (length(d.level) <= 5) {
    ...
    id <- rev(id)
    p <- ggplot(data, aes(x = period, y = units, fill = res), 
                position = "identity")
    if (gridOff == FALSE) {
      p <- p + geom_tile(colour = grid.color, size = 0.1, 
                         stat = "identity")
    }
    else {
    ...
    if (length(d.level) <= 5) {
      p <- p + scale_fill_manual(NA, breaks = breaks, 
                                 values = col, labels = label)
    }
    else {
      p <- p + scale_fill_gradient(low = col[1], high = col[2]) + 
        guides(fill = guide_legend(title = label))
    }
    p <- p + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
                   panel.border = element_rect(fill = NA, color = border.color, 
                                               size = 0.5, linetype = "solid"), axis.line = element_blank(), 
                   axis.ticks = element_blank(), axis.title = element_text(size = cex.lab), 
                   axis.title.x = element_text(margin = margin(t = 8, 
                                                               r = 0, b = 0, l = 0)), axis.title.y = element_text(margin = margin(t = 0, 
                                                                                                                                  r = 8, b = 0, l = 0)), axis.text = element_text(color = "black", 
                                                                                                                                                                                  size = cex.axis), axis.text.x = element_text(size = cex.axis, 
                                                                                                                                                                                                                               angle = angle, hjust = x.h, vjust = x.v), axis.text.y = element_text(size = cex.axis), 
                   plot.background = element_rect(fill = background.color), 
                   legend.background = element_rect(fill = legend.color), 
                   legend.position = legend.pos, legend.margin = margin(c(0, 
                                                                          5, 5, 0)), legend.text = element_text(margin = margin(r = 10, 
                                                                                                                                unit = "pt"), size = cex.legend), plot.title = element_text(size = cex.main, 
                                                                                                                                                                                            hjust = 0.5, face = "bold", margin = margin(8, 
                                                                                                                                                                                                                                        0, 8, 0)))
    if (length(d.level) <= 5) {
    ...
    if (axis.lab == "both") {
      p <- p + scale_x_continuous(expand = c(0, 0), breaks = T.b, 
                                  labels = time.label[T.b]) + scale_y_continuous(expand = c(0, 
                                                                                            0), breaks = N.b, labels = id[N.b])
    }
    else if (axis.lab == "unit") {
      p <- p + scale_x_continuous(expand = c(0, 0), breaks = T.b, 
                                  labels = NULL) + scale_y_continuous(expand = c(0, 
                                                                                 0), breaks = N.b, labels = id[N.b])
    }
    else if (axis.lab == "time") {
      p <- p + scale_x_continuous(expand = c(0, 0), breaks = T.b, 
                                  labels = time.label[T.b]) + scale_y_continuous(expand = c(0, 
                                                                                            0), breaks = N.b, labels = NULL)
    }
    else if (axis.lab == "off") {
      p <- p + scale_x_continuous(expand = c(0, 0), breaks = 1:length(show), 
                                  labels = NULL) + scale_y_continuous(expand = c(0, 
                                                                                 0), breaks = 1:N, labels = NULL)
    }
